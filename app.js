            ...
            recibidasContainer.innerHTML = citasRecibidas.map(cita => {
                ...
                if (cita.estatus_cita === 'Pendiente') {
                    ...
                    if (cita.estatus_cita === 'Trato Cerrado') {
                        botones = `
                            <button onclick="generarContratoPDF('${cita.id}')" class="btn btn-sm btn-dark w-100 py-2 mb-1 fw-bold shadow-sm" title="Descargar Contrato"><i class="bi bi-file-earmark-pdf-fill me-1"></i> Ver Contrato</button>
                            ${numeroLimpio ? `<a href="https://wa.me/52${numeroLimpio}" target="_blank" class="btn btn-sm btn-success w-100 py-2"><i class="bi bi-whatsapp"></i> Contactar</a>` : ''}
                        `;
                    } else {
                        botones = `<span class="badge bg-dark w-100 py-2">${cita.estatus_cita}</span>`;
                    }
                }
    ...
    if (errCitas) {
        ...
        citasContainer.innerHTML = citas.map(cita => {
            ...
            const btnRevisarContra = (cita.estatus_cita === 'Contraoferta Recibida')
                ...
                : '';
            const btnContrato = (cita.estatus_cita === 'Trato Cerrado')
                ? `<button onclick="generarContratoPDF('${cita.id}')" class="btn btn-sm btn-success fw-bold w-100 mt-1 shadow-sm"><i class="bi bi-file-earmark-pdf-fill"></i> Descargar Contrato</button>`
                : '';
...
// ==========================================
// --- GENERADOR DE CONTRATO DE COMPRAVENTA PDF ---
// ==========================================
// ==========================================
// --- GENERADOR DE CONTRATO DE COMPRAVENTA PDF ---
// ==========================================
window.generarContratoPDF = async function (citaId) {
    try {
        // 1. Traer todos los datos cruzados de la cita
        const { data: cita, error } = await supabaseClient
            ...
            .select(`
                    ...
                    mensaje_oferta,
                    respuesta_admin,
                    perfiles!citas_cliente_cliente_id_fkey(nombre_completo), 
                    solicitudes_publicacion(titulo, precio, ubicacion, perfiles(nombre_completo))
                `)
            .eq('id', citaId)
            .single();

        if (error || !cita) throw new Error("No se pudieron cargar los datos para el contrato.");

        // 2. Extraer variables limpias de nombres y propiedad
        const nombreComprador = cita.perfiles?.nombre_completo || 'El Comprador';
        const nombreVendedor = cita.solicitudes_publicacion?.perfiles?.nombre_completo || 'El Vendedor';
        const propiedadTitulo = cita.solicitudes_publicacion?.titulo || 'Inmueble Especificado';
        const propiedadUbicacion = cita.solicitudes_publicacion?.ubicacion || 'Ubicación acordada';

        // --- LÓGICA DE PRECIO DINÁMICO ---
        const mensajeOferta = cita.mensaje_oferta || "";
        const respuestaVendedor = cita.respuesta_admin || "";

        // Función rápida para extraer el primer número grande que parezca dinero (5 o más dígitos)
        const extraerPrecio = (texto) => {
            const coincidencia = texto.replace(/,/g, '').match(/\d{5,}/);
            return coincidencia ? coincidencia[0] : null;
        };

        // Prioridad: 1. Lo que respondió el dueño -> 2. Lo que ofreció el cliente -> 3. Precio de catálogo
        const precioNegociado = extraerPrecio(respuestaVendedor) || extraerPrecio(mensajeOferta);
        const precioFinalCalculado = precioNegociado ? Number(precioNegociado) : Number(cita.solicitudes_publicacion?.precio);

        const precioOficial = precioFinalCalculado.toLocaleString('es-MX');
        // ---------------------------------
        ...
        doc.setTextColor(26, 42, 58); // Color Navy de Innova
        doc.text("INNOVA", 105, 20, null, null, "center");

        doc.setFontSize(14);
        doc.text("CONTRATO DE COMPRAVENTA DE INMUEBLE", 105, 30, null, null, "center");
        ...
        const parrafo1 = `Conste por el presente documento, el CONTRATO DE COMPRAVENTA que celebran por una parte ${nombreVendedor}, a quien en lo sucesivo se le denominará "EL VENDEDOR", y por la otra parte ${nombreComprador}, a quien en lo sucesivo se le denominará "EL COMPRADOR", el cual sujetan a las siguientes declaraciones y cláusulas:`;

        const parrafo2 = `PRIMERA: "EL VENDEDOR" declara ser legítimo propietario del inmueble conocido en la plataforma INNOVA como "${propiedadTitulo}", ubicado en ${propiedadUbicacion}.`;

        const parrafo3 = `SEGUNDA: "EL COMPRADOR" declara que es su intención adquirir el inmueble descrito en la cláusula anterior, aceptando las condiciones físicas y legales en las que se encuentra.`;

        const parrafo4 = `TERCERA: El precio pactado y aceptado para el cierre de esta operación a través del sistema de ofertas de la plataforma es de $${precioOficial} MXN.`;

        const parrafo5 = `CUARTA: Las partes acuerdan formalizar este compromiso de manera física y legal ante el notario público de su elección en un plazo no mayor a 30 días naturales a partir de la fecha de este documento.`;

        // splitTextToSize ayuda a que el texto baje a la siguiente línea y no se salga de la hoja
        const lines1 = doc.splitTextToSize(parrafo1, 170);
        const lines2 = doc.splitTextToSize(parrafo2, 170);
        const lines3 = doc.splitTextToSize(parrafo3, 170);
        const lines4 = doc.splitTextToSize(parrafo4, 170);
        const lines5 = doc.splitTextToSize(parrafo5, 170);

        let y = 45;
        doc.text(lines1, 20, y); y += (lines1.length * 6) + 5;
        doc.text(lines2, 20, y); y += (lines2.length * 6) + 5;
        doc.text(lines3, 20, y); y += (lines3.length * 6) + 5;
        doc.text(lines4, 20, y); y += (lines4.length * 6) + 5;
        doc.text(lines5, 20, y); y += (lines5.length * 6) + 15;

        // Fecha y cierre
        doc.text(`Firmado simbólicamente en la plataforma INNOVA el día: ${fechaActual}.`, 20, y);
        y += 30;

        // Firmas
        doc.line(30, y, 90, y); // Línea Vendedor
        doc.line(120, y, 180, y); // Línea Comprador

        doc.setFontSize(10);
        doc.text("Firma de EL VENDEDOR", 60, y + 5, null, null, "center");
        doc.text("Firma de EL COMPRADOR", 150, y + 5, null, null, "center");

        doc.setFontSize(9);
        doc.setFont("helvetica", "italic");
        doc.text(nombreVendedor, 60, y + 10, null, null, "center");
        doc.text(nombreComprador, 150, y + 10, null, null, "center");

        // Footer Innova
        doc.setFontSize(8);
        doc.setTextColor(150, 150, 150);
        doc.text("Este documento es un comprobante de acuerdo generado por INNOVA.", 105, 280, null, null, "center");

        // Descargar PDF
        doc.save(`Contrato_Innova_${citaId.substring(0, 6)}.pdf`);

    } catch (error) {
        console.error("Error al generar contrato:", error);
        alert("Hubo un problema al generar el contrato: " + error.message);
    }
};